...
class PUFWrapper:
    ...
    def create_input_outputs(self, num_crps, add_reliabilty_to_output, add_NFPr_to_output, 
                             num_repetitions_to_evaluate=11, num_NFPr_samples=16, num_flips_for_NFPr=1,
        ...
        if add_NFPr_to_output:
            NonFlippingProbability=self.current_puf.compute_NonFlippingProbability_response(challenges, nSamples=num_NFPr_samples,
                                                                                             FlipNum=num_flips_for_NFPr,
                                                                                             input_is_feature_vector=self.create_feature_vectors )
            output = np.stack([responses, NonFlippingProbability], -1)
            print("currecnt process: add_NFPr_to_output ")
        elif add_reliabilty_to_output:
            ...
            print("currecnt process: add_reliabilty_to_output")
            output = np.stack([responses, reliability], -1)
        else:
            output = responses
            print("currecnt process: add response to output")

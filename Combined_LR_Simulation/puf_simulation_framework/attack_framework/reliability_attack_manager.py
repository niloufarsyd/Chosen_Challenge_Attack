...
class ReliabilityAttackManager(AttackManager):
    ...
    def create_inputs_outputs(self, puf_wrapper, attack_parameters):
        ...
        perform_NFPr_attack=attack_parameters.get('perform_NFPr_attack', True)
        reliability_repetitions = attack_parameters.get('reliability_repetitions', 0)
        nSamples = attack_parameters.get('nSamples', 16)
        FlipNum = attack_parameters.get('num_flips_for_NFPr', 1)
    ...
    def estimate_reliability(self, puf_wrapper, inputs, outputs, attack_parameters):
        ...
        print("shape:  ",outputs['validation_noisy'].shape)
        validation_noisy_responses = outputs['validation_noisy'][:,0] # it was [:,0]
        ...
        print("noisy to noisy Validation in reliability_attack_manager....")
        _, second_valiation_noisy_responses = puf_wrapper.create_input_outputs(0,
                                                                            add_NFPr_to_output=False,
